...
@property (nonatomic,strong)NSMutableArray *reusablePhotoItemViewSetM;
...
//    if([self.visiblePhotoItemViewDictM objectForKey:@(page)] != nil) return;
    ...
    __block NSMutableArray *pageTOload = [@[@(page-1),@(page),@(page + 1)] mutableCopy];
    __block NSMutableArray *viewToEnqueue = [NSMutableArray array];
    [self.visiblePhotoItemViewDictM enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
                        PhotoItemView *itemview = obj;
                       
                        if (!itemview.pageIndex || ![pageTOload containsObject:@(itemview.pageIndex)]){
                            [viewToEnqueue addObject:itemview];
                        }else if (itemview.pageIndex){
                            [pageTOload removeObject:@(itemview.pageIndex)];
                        }

    }];
    
    for (PhotoItemView *itemview in viewToEnqueue){
        [itemview removeFromSuperview];
        [self.visiblePhotoItemViewDictM removeObjectForKey:@(itemview.pageIndex)];
        [self.reusablePhotoItemViewSetM addObject:itemview];
    }
    
    for (NSNumber *page in pageTOload) {
        [self addItemViewForPage:page.integerValue];
    }
    
}


-(void)addItemViewForPage:(NSInteger)page
{
    if (page < 0 || page >= self.pageCount){
        return;
    }
    
    PhotoItemView *itemview = [self dequeReusablePhotoItemView];
    itemview.pageIndex = page;
    itemview.type = self.type;
    itemview.photoModel = self.photoModels[page];
    [self.scrollView addSubview:itemview];
    [self.visiblePhotoItemViewDictM setObject:itemview forKey:@(page)];
    itemview.ItemViewSingleTapBlock = ^(){
            [self singleTap];
    };
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    page = MAX(page, 0);
    page = MIN(page, self.pageCount -1);
    self.page = page;
...
-(PhotoItemView *)dequeReusablePhotoItemView{
    ...
    if (self.reusablePhotoItemViewSetM.count>0){
         photoItemView = [self.reusablePhotoItemViewSetM objectAtIndex:0];
...
/** 可重用集合 */
-(NSMutableArray *)reusablePhotoItemViewSetM{
    
    if(_reusablePhotoItemViewSetM ==nil){
        _reusablePhotoItemViewSetM =[NSMutableArray array];
    }
